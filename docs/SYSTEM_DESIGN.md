...
│  ├── llm.py            — get_llm_client(user_id, db)    │
│  │                        per-user → global fallback    │
│  └── user_settings.py  — encrypt/decrypt API keys       │
└──────┬──────────────────────┬───────────────────────────┘
...
### user_model_settings (Sprint 10)
```
id          UUID PK
user_id     UUID FK → users (CASCADE) UNIQUE  -- один набор настроек на юзера
provider    VARCHAR(50)    -- 'anthropic' | 'openai' | 'ollama'
model_name  VARCHAR(100)   -- 'claude-sonnet-4-6', 'gpt-4o', 'llama3.2:3b'
api_key_enc TEXT           -- Fernet-шифрованный ключ (NULL для ollama без ключа)
base_url    TEXT           -- кастомный URL (для ollama, напр. http://host:11434)
created_at  TIMESTAMPTZ
updated_at  TIMESTAMPTZ
```

**Безопасность:** API-ключ никогда не хранится в открытом виде. При записи шифруется через `cryptography.fernet` с серверным `ENCRYPTION_KEY`. GET-ответ возвращает только замаскированное значение (`sk-ant-...••••••••`).
...
## 5. LLM-провайдеры и маршрутизация

### 5.1 Глобальные провайдеры (env-конфигурация)

Системные дефолты, применяются когда у пользователя нет персональных настроек:
...
### 5.2 Персональные настройки модели (Sprint 10)

Начиная со Sprint 10 каждый пользователь может переопределить глобальные настройки, задав свой провайдер и API-ключ через UI или API.

**Логика разрешения провайдера при запросе:**

```
Запрос к агенту / чату (user_id известен)
      │
      ▼
get_llm_client(user_id, db)
  1. SELECT user_model_settings WHERE user_id = ?
  2. Если запись есть:
     a. decrypt(api_key_enc) → api_key
     b. Создать клиент (Anthropic / OpenAI / Ollama) с ключом пользователя
     c. Вернуть клиент
  3. Если записи нет:
     a. Использовать глобальный settings.llm_provider + settings.anthropic_api_key
     b. Вернуть глобальный клиент
```

**Поддерживаемые провайдеры в пользовательских настройках:**

| Провайдер | Требует ключ | Примеры моделей | Базовый URL |
|-----------|-------------|-----------------|-------------|
| `anthropic` | Да | claude-sonnet-4-6, claude-opus-4-7, claude-haiku-4-5 | — |
| `openai` | Да | gpt-4o, gpt-4o-mini, o3-mini | — |
| `ollama` | Нет | llama3.2:3b, mistral, codellama | http://host:11434 |

**Шифрование ключей:**
- Алгоритм: Fernet (AES-128-CBC + HMAC-SHA256, симметричное)
- Ключ шифрования: `ENCRYPTION_KEY` в env (base64-encoded 32 bytes)
- Генерация: `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`
- Хранится только зашифрованный ключ, plaintext не логируется и не кешируется
- GET `/api/settings/model` возвращает маску: `sk-ant-api03-...••••••••`
...
| GET | /api/settings/model | Получить настройки модели пользователя (ключ замаскирован) |
| PUT | /api/settings/model | Сохранить настройки модели (ключ шифруется) |
| POST | /api/settings/model/test | Проверить API-ключ у провайдера перед сохранением |
...
|-----------|--------|---------|
| Нет аутентификации | ✅ Решено в Sprint 9 | JWT + refresh tokens |
| Глобальный API-ключ в env | ✅ Решено в Sprint 10 | Per-user Fernet-шифрованные ключи |
...
| `ENCRYPTION_KEY` ротация | ⚠️ | При смене ключа — re-encrypt все `api_key_enc` в БД или попросить юзеров ввести ключи заново |
...
| LLM | Anthropic / OpenAI / Ollama (per-user) | — |
| Key encryption | cryptography (Fernet) | 43.0 |
